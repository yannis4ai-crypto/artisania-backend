...
async def send_sms(*, to: str, body: str) -> None:
    if not settings.sinch_api_token:
        raise RuntimeError("SINCH_API_TOKEN manquant — configurer .env avant l'envoi")
    ...
    async with httpx.AsyncClient() as client:
        response = await client.post(
            url,
            json=payload,
            headers={"Authorization": f"Bearer {settings.sinch_api_token}"},
        )
